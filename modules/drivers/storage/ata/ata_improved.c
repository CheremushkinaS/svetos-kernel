...
static uint64_t ata_get_capacity(void* device) {
    // For now, return a fixed capacity
...
static int ata_ioctl(void* device, uint32_t cmd, void* arg) {
    switch (cmd) {
        case 1: // Get device info
            printk("ATA: IOCTL get device info\n");
            return 0;
        case 2: // Flush cache
            printk("ATA: IOCTL flush cache\n");
            return 0;
        default:
            printk("ATA: Unknown IOCTL command %d\n", cmd);
            return -1;
    }
}
...
static void ata_init(void) {
    printk("ATA Disk Driver initialized\n");
