...
// Простые функции для отладки - будут заменены на printk при загрузке
static void debug_msg(const char* msg) {
    // Временная заглушка - в реальной системе заменится на printk
    (void)msg;
}
...
static uint64_t ata_get_capacity(void* device) {
    (void)device; // Unused parameter
    return 32 * 1024 * 1024; // 32MB для демонстрации
}
...
static void ata_register_interfaces(void) {
    // Эта функция будет вызвана ядром для регистрации интерфейсов
